...
function renderCharacterList(characterArray, targetElementId) {
...
function renderFilteredByAge(characterArray, ageThreshold, targetElementId) {
...
renderFilteredByAge(characters, 50, "age-filter-list");
...
function renderListWithErrorHandling(characterArray, targetElementId) {
  const targetElement = document.getElementById(targetElementId);
  ...
  characterArray.forEach((character, index) => {
    if (character.name) {
      ...
      const errorMessage = `Error: Character at Index ${index} is missing a name property`;
...
renderListWithErrorHandling(characters, "error-handling-list");
...
function renderBrokenArrayTest(characterArray, targetElementId) {
  const targetElement = document.getElementById(targetElementId);
...
const brokenCharacters = [
  ...
  { id: 3, name: "Princess Leia", age: 23 },
  { id: 4, age: 57 }, // Missing name
  { id: 5, name: "Yoda", age: 900 },
];

renderBrokenArrayTest(brokenCharacters, "broken-array-list");